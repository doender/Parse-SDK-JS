    ...
    value: function (options) {
      ...
      if (!this._previousSave) {
        if (this._source.format === 'file') {
          this._previousSave = controller.saveFile(this._name, this._source, options.progress).then(function (res) {
            _this._name = res.name;
            _this._url = res.url;
            return _this;
          });
        } else {
          this._previousSave = controller.saveBase64(this._name, this._source, options.progress).then(function (res) {
            _this._name = res.name;
            _this._url = res.url;
            return _this;
          });
...
var DefaultController = {
  saveFile: function (name, source, progress) {
    ...
    url += 'files/' + name;
    return _CoreManager2.default.getRESTController().ajax('POST', url, source.file, headers, progress);
  },
